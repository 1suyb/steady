...
- 인터페이스의 특징에 대해 정리하고 구현된 로직을 분석
	- 인터페이스의 특징
		...
		- 이 인터페이스를 상속받은 클래스는 이 행동이 꼭 구현되어 있다! 이 행동을 할 수 있다!를 보장함
	- IDamagable 로직 분석
		- IDmagable를 상속받은 클래스에는 TakePhysicalDamage를 구현해야함.
		- CampFire에서 데미지를 가할때 IDmagable를 상속받은 클래스를 찾아 그 클래스의 TakePhysicalDamage를 호출하여 데미지를 입힘
- 핵심 로직을 분석
	- CampFire
		- 닿은 오브젝트들의 IDamagable을 상속받은 클래스가 있으면 저장해두고
		- 일정 간격으로 저장해둔 오브젝트들의 IDamagable의 TakePhysicalDamage를 호출함
		- 떨어지면 오브젝트를 지워서 호출하지 않도록 함
	- DamageIndicator
		- 캐릭터가 피해를 입을 때마다 호출해주는 onTakeDamage에 Falsh를 구독하여 캐릭터가 피해를 입을 때 indicator가 반짝(?)일 수 있게 해줌.
		- Flash는 코루틴을 이용해 매 프레임 점점 투명해지도록 함

## Q3
### 분석 문제
- Interaction 기능의 구조와 핵심 로직 분석
	- 일정 시간마다 화면 중앙에서 레이를 쏘아 인터렉션 가능한 오브젝트가 맞으면 프롬프트를 보여줌
	- 이때, 일정시간 전부터 지금까지 계속 같은 오브젝트가 맞고 있는데 프롬프트를 보여주는 작업을 계속 하면 낭비이므로 같은 오브젝트가 맞고 있는건지 검사한번 해줌
	- interaction 입력을 받으면 해당 오브젝트의OnInteract를 호출
- Inventory 기능의 구조와 핵심 로직 분석
	- 기능 - 아이템 보여주기 / 아이템 획득 하기 / 아이템 사용하기 / 아이템 버리기
	- 슬롯에서 아이템 데이터를 들고 있고, 아이템을 보여줌
	- UI 인벤토리에서 아이템을 획득하면 슬롯에 데이터를 넣어주고
	- 아이템 슬롯에서 자기가 클릭됨을 감지하면 인벤토리에 자기가 선택되었다고 인벤토리의 함수를 호출함
	- 아이템 슬롯에서 인벤토리의 selectItem을 호출해주면, 해당 아이템이 선택되었고, 아이템의 타입에 따라 버튼을 보여줌
	- 버리기를 누르면 캐릭터의 드랍아이템 위치에 아이템 오브젝트를 생성하고, 슬롯에서 차감시킴
	- 사용을 누르면 아이템 타입에 따라 hp를 채우거나 허기를 채움
	- 착용을 누르면 아이템을 착용함